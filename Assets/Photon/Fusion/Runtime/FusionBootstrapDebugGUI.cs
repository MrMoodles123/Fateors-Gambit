namespace Fusion
{
    ...
    float ms = 4;
        ...
        {
            if (TryGetComponent<FusionBootstrapDebugGUIS>(out var gui) == false)
            {
                gui = gameObject.AddComponent<FusionBootstrapDebugGUIS>();
            }
            gui.enabled = true;
            this.enabled = false;
        }
        ...
        protected virtual void OnGUI()
        {
            ...
            GUILayout.BeginArea(new Rect(Screen.width / 2 + 30 * margin, padding * 13, width - 15 * margin, Screen.height));
            {
                GUILayout.BeginVertical(GUI.skin.window);
                {
                    ...
                    {
                        var stagename = _nicifiedStageNames.TryGetValue(nds.CurrentStage, out var stage) ? stage : "<size=50>Unrecognized Stage</size>";
                ...
                GUILayout.BeginVertical(GUI.skin.window);
                {
                    ...
                    if (currentstage == FusionBootstrap.Stage.Disconnected)
                    {
                        ...
                        if (GUILayout.Button(EnableHotkeys ? "<size=70>Matchmake</size>" : "<size=70>Matchmake</size>", GUILayout.Height(height)))
                        {
                            if (_isMultiplePeerMode)
                            {
                                StartMultipleAutoClients(nds);
                        ...
                        if (GUILayout.Button(EnableHotkeys ? "<size=70>Settings</size>" : "<size=70>Settings</size>", GUILayout.Height(height)))
                        {
                            ...
                            if (TryGetComponent<FusionBootstrapDebugGUIS>(out var guiS) == false)
                            {
                                guiS = gameObject.AddComponent<FusionBootstrapDebugGUIS>();
                            }
                            guiS.enabled = true;
                            gui.enabled = false;
                        }

                        if (GUILayout.Button(EnableHotkeys ? "<size=70>Tutorial</size>" : "<size=70>Tutorial</size>", GUILayout.Height(height)))
                        {
                            if (TryGetComponent<FusionBootstrapDebugGUI>(out var gui) == false)
                            {
                                gui = gameObject.AddComponent<FusionBootstrapDebugGUI>();
                        ...
                        if (GUILayout.Button(EnableHotkeys ? "<size=70>Restart Game</size> (R)" : "<size=70>Restart Game</size>", GUILayout.Height(height)))
                        {
                            RestartApplication();
                        }

                        if (GUILayout.Button(EnableHotkeys ? "<size=70>Exit Game</size>" : "<size=70>Exit Game</size>", GUILayout.Height(height)))
                        {
