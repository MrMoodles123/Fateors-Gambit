namespace Fusion {
  ...
  public class FusionBootstrap : Fusion.Behaviour {
    ...
    private static FusionBootstrap instance;
    /// <summary>
    ...
    /// <summary>
    /// <see cref="FusionBootstrapDebugGUIS"/> will render GUI elements while <see cref="CurrentStage"/> == <see cref="Stage.Settings"/>.
    /// </summary>
    [InlineHelp]
    [DrawIf(nameof(StartMode), (long)StartModes.UserInterface, Hide = false)]
    public bool AutoHideGUIS = true;
    ...
    public bool AutoConnectVirtualInstances = false;
    /// <summary>
    ...
    protected bool ShowAutoClients    => UsingMultiPeerMode && (StartMode == StartModes.UserInterface || (StartMode == StartModes.Automatic && AutoStartAs != GameMode.Single));
        ...
        void Awake()
        {
            //Invoke(nameof(HandleInstance), 2.5f); 
        }
        ...
        void HandleInstance()
        {
            ...
            if (objs.Length > 1)
            {
                // Sort by instance ID (higher ID means newer object)
                System.Array.Sort(objs, (a, b) => b.GetInstanceID().CompareTo(a.GetInstanceID()));

                // Destroy the newest instance (first in the sorted list)
                for (int i = 0; i < objs.Length; i++) 
                {
                    FusionBootstrap other = objs[i].GetComponent<FusionBootstrap>();
                    if (string.IsNullOrEmpty(other.InitialScenePath))
                    {
                        Destroy(objs[i]);
                    }
                }
            ...
            // Keep older instances and apply DontDestroyOnLoad
    ...
    protected virtual void Start() {
      ...
      am.SetFloat("master", PlayerPrefs.GetFloat("Volume"));
